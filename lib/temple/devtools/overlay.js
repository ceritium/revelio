(function() {
  ...
  function createOverlayLabel(element, type) {
    if (Array.prototype.some.call(element.children, function(c) { return c.classList.contains('temple-devtools-overlay-label'); })) return;

    var file = element.getAttribute('data-devtools-file') || '';
    var line = element.getAttribute('data-devtools-line') || '';
    if (!file) return;

    var colors = TYPE_COLORS[type] || TYPE_COLORS.view;
    var name = shortName(file);
    var relativePath = file + ':' + line;
    ...
    label.addEventListener('mouseenter', function() {
      ...
      document.querySelectorAll('.temple-devtools-overlay-label').forEach(function(l) {
        l.style.zIndex = '1000';
      });
    ...
    // Position inside the element if overflow would clip the label
    var parentOverflow = element.parentElement ? window.getComputedStyle(element.parentElement).overflow : 'visible';
    var rect = element.getBoundingClientRect();
    if (rect.top < 20 || parentOverflow === 'hidden' || parentOverflow === 'auto' || parentOverflow === 'scroll') {
      label.style.top = '2px';
  ...
  function removeOverlayLabel(element) {
    Array.prototype.forEach.call(element.children, function(c) {
      if (c.classList.contains('temple-devtools-overlay-label')) c.remove();
    });
  }
  ...
  function createBoundaryOverlay(boundary) {
    ...
    label.textContent = boundary.short;

    label.addEventListener('mouseenter', function() {
      label.textContent = boundary.file;
      label.style.zIndex = '1002';
    });
    label.addEventListener('mouseleave', function() {
      label.textContent = boundary.short;
  ...
  function startStimulusObserver() {
    if (stimulusObserver) return;
    var debounceTimer = null;
    stimulusObserver = new MutationObserver(function() {
      clearTimeout(debounceTimer);
      debounceTimer = setTimeout(runStimulusLinter, 300);
    });
    stimulusObserver.observe(document.body, {
      childList: true, subtree: true,
      attributes: true, attributeFilter: ['data-controller', 'data-action']
    });
  }

  function stopStimulusObserver() {
    if (stimulusObserver) {
      stimulusObserver.disconnect();
      stimulusObserver = null;
    }
  }
  ...
  function setupToggles() {
    ...
    bindToggle('templeToggleStimulusLinter', 'showStimulusLinter', function() {
      if (state.showStimulusLinter) {
        ...
        startStimulusObserver();
      } else {
        runStimulusLinter();
        stopStimulusObserver();
      }
    });
