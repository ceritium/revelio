...
<div class="temple-devtools-floating-menu">\
  ...
  var globalObserver = new MutationObserver(function(mutations) {
    ...
    var dominated = mutations.every(function(m) {
      ...
      if (t.closest && (t.closest('.temple-devtools-boundary-overlay') ||
        t.closest('.temple-devtools-floating-menu') ||
        t.closest('.temple-devtools-tooltip') ||
        t.closest('.temple-devtools-lint-overlay'))) return true;
      // Ignore our overlays being added/removed from body
      if (m.type === 'childList') {
        var dominated = true;
        m.addedNodes.forEach(function(n) {
          if (n.nodeType === 1 && n.className && typeof n.className === 'string' &&
              n.className.indexOf('temple-devtools-') === 0) return;
          dominated = false;
        });
