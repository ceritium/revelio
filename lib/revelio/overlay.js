(function() {
  ...
  function logTurboStream(action, target, template) {
    turboStreamLog.push({
      action: action,
      target: target,
      template: template || '',
    ...
    renderTurboStreamLog();
  }

  function renderTurboStreamLog() {
    var container = document.getElementById('revelioTurboStreamLog');
    if (!container) return;

    if (turboStreamLog.length === 0) {
      container.innerHTML = '';
      return;
    }

    var html = '<details class="revelio-section" open>';
    html += '<summary>Streams (' + turboStreamLog.length + ')</summary>';
    html += '<div class="revelio-stream-list">';
    turboStreamLog.forEach(function(s) {
      html += '<div class="revelio-stream-item">';
      html += '<span class="revelio-lint-badge revelio-lint-badge-' + s.action + '">' + s.action + '</span> ';
      html += '<span class="revelio-stream-target">' + escapeHtml(s.target) + '</span>';
      if (s.template) {
        html += ' <span class="revelio-stream-template">' + escapeHtml(s.template) + '</span>';
      }
      html += '<span class="revelio-stream-time">' + s.time + '</span>';
      html += '</div>';
    });
    html += '</div></details>';
    container.innerHTML = html;
  ...
  function setupMenu() {
    ...
    document.addEventListener('click', function(e) {
      ...
      if (menu && !menu.contains(e.target) && state.menuOpen) {
        // Don't close panel when clicking navigation links — Turbo will replace the page
  ...
  function scheduleRefresh() {
    ...
    refreshTimer = setTimeout(function() {
      refreshTimer = null;
      // Re-inject menu if it was removed (e.g., by Turbo stream replacing a parent)
      injectStyles();
      injectMenu();
      setupMenu();
      setupToggles();
  ...
  document.addEventListener('turbo:before-stream-render', function(event) {
    ...
    templates.forEach(function(stream) {
      // Log the stream
      var action = stream.getAttribute('action') || 'unknown';
      var target = stream.getAttribute('target') || stream.getAttribute('targets') || '';
      var tpl = stream.querySelector('template');
      var templateFile = '';
      if (tpl && tpl.content) {
        var el = tpl.content.querySelector('[data-revelio-file]');
        if (el) {
          templateFile = el.getAttribute('data-revelio-file');
          // Inject comment markers
          var type = el.getAttribute('data-revelio-type') || 'view';
          var short = templateFile.split('/').pop();
          var begin = document.createComment(' revelio-begin file="' + templateFile + '" type="' + type + '" short="' + short + '" ');
          var end = document.createComment(' revelio-end file="' + templateFile + '" ');
          tpl.content.insertBefore(begin, tpl.content.firstChild);
          tpl.content.appendChild(end);
        }
      }
      logTurboStream(action, target, templateFile);
    });
